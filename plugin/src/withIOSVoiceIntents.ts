 ...
 *
 * Two non-negotiable rules iOS enforces (we enforce them earlier with
 * a clear error so developers don't see silent linkd rejections):
 *
 ...
 *    (`String`, `Int`, `Double`, `Date`, etc.). Per Apple DTS engineer
 *    Ed Ford: *"Invalid parameter type. AppEntity and AppEnum are the
 *    only allowed types"* (https://developer.apple.com/forums/thread/770037).
 *    We throw at prebuild on any primitive slot because linkd would
 *    silently skip the phrase otherwise — see issue #37.
 *
 * `${applicationName}` is emitted as the raw Swift interpolation
 * `\(.applicationName)` (un-escaped: it's compile-time syntax, not a
 * runtime character).
 */
function buildPhraseLiteral(
  ...
  // Reject any other ${...} slot — primitives can't be slotted (#37),
  // and we don't support entity/enum slots yet (#28 / #29 unlock that).
  const TOKEN_RE = /\$\{([a-zA-Z_][a-zA-Z0-9_]*)\}/g;
  const declaredNames = new Set(declaredParams.map((p) => p.name));
  let m: RegExpExecArray | null;
  while ((m = TOKEN_RE.exec(phrase)) !== null) {
    const token = m[1];
    if (token === "applicationName") continue;
    if (declaredNames.has(token)) {
      throw new Error(
        `[expo-assistant] Phrase for "${intentId}" references "\${${token}}" as a voice slot, but "${token}" is a primitive-typed parameter. ` +
          ...
          `Use a bare phrase (e.g. "Verb in \${applicationName}") and rely on requestValueDialog to prompt for "${token}". Tracked in #37.`
      );
    }
    throw new Error(
      `[expo-assistant] Phrase for "${intentId}" references undeclared parameter "\${${token}}". ` +
        ...
        `Note: voice slots only work for AppEntity / AppEnum types (#37), not primitives.`
    );
  }

  // Only ${applicationName} interpolates today. Split + escape + rejoin.
  const parts = phrase.split(APP_NAME_TOKEN).map(escapeSwift);
  return `"${parts.join("\\(.applicationName)")}"`;
}
