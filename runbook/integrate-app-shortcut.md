...
| `schema` | `string` | no | `undefined` | AssistantSchemas conformance (iOS 18+). Emits `@AppIntent(schema: .<schema>)` and makes the intent eligible for Apple Intelligence AI-surfaced contexts (Spotlight tiles, onscreen-content suggestions, contextual app gallery). **Does NOT enable Siri voice triggering** — empirically verified on iPhone 16 Pro + iOS 26.2 with AI enabled: voice phrases targeting schema intents fall through to web search. For voice-triggered actions use a vanilla AppIntent with explicit `phrases[]`. Known schemas: `"system.search"`. **See `runbook/integrate-assistant-schema.md` for the full guide + the voice/schema compose pattern.** |
| `assistantOnly` | `boolean` | no | `false` | When `true`, emits `static let isAssistantOnly = true` — intent vanishes from Shortcuts.app Library + Spotlight; surfaced only via Apple Intelligence reasoning paths. Only meaningful when `schema` is also set. Use during schema-migration scenarios OR in the compose pattern (vanilla intent for voice + schema intent with `assistantOnly: true` for AI surfaces). |
