...
| `schema` | `string` | no | `undefined` | AssistantSchemas conformance (iOS 18+). When set, the intent gets `@AppIntent(schema: .<schema>)` macro applied and Apple Intelligence can route to it via semantic intent inference. Known schemas: `"system.search"`. **See `runbook/integrate-assistant-schema.md` for the full schema guide.** |
| `assistantOnly` | `boolean` | no | `false` | When `true`, emits `static let isAssistantOnly = true` — intent vanishes from Shortcuts.app Library + Spotlight; only Apple Intelligence can route to it. Only meaningful when `schema` is also set. Use during schema-migration scenarios. |
