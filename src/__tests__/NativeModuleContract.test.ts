...
describe("ExpoAssistantModule contract", () => {
  ...
  it.each(expectedMethods)("exposes %s as a function", (method) => {
    expect(
      typeof (ExpoAssistantModule as Record<string, unknown>)[method]
    ).toBe("function");
  });

  it("exposes the event emitter surface", () => {
    expect(
      typeof (ExpoAssistantModule as Record<string, unknown>).addListener
    ).toBe("function");
    expect(
      typeof (ExpoAssistantModule as Record<string, unknown>).removeListeners
    ).toBe("function");
  });
});
