...
describe('VoiceAssistant', () => {
  ...
  describe('Intent Registration', () => {
    ...
    it('should handle registration failure', async () => {
      ...
      mockModule.registerIntent.mockRejectedValue(new Error('Registration failed'));

      await expect(voiceAssistant.registerIntent(intent)).rejects.toThrow('Registration failed');
    ...
    it('should throw when unregistering non-existent intent', async () => {
      await expect(voiceAssistant.unregisterIntent('non-existent'))
  ...
  describe('Intent Execution', () => {
    ...
    it('should execute intent with parameters', async () => {
      ...
      const result = await voiceAssistant.executeIntent('search-intent', { query: 'test' });
    ...
    it('should handle execution errors', async () => {
      const intent = VoiceIntentBuilder
        ...
        .withCategory(IntentCategory.CUSTOM)
        .withHandler({
          handle: async () => { throw new Error('Execution failed'); }
        })
        .build();
    ...
    it('should use error handler when provided', async () => {
      const intent = VoiceIntentBuilder
        ...
        .withCategory(IntentCategory.CUSTOM)
        .withHandler({
          handle: async () => { throw new Error('Expected error'); },
      ...
      const result = await voiceAssistant.executeIntent('handled-error-intent', {});
    ...
    it('should throw when executing non-registered intent', async () => {
      await expect(voiceAssistant.executeIntent('unknown', {}))
        .rejects.toThrow('Intent unknown not registered');
    });
  });
  ...
  describe('Intent Donation', () => {
    ...
    it('should donate intent for Siri suggestions', async () => {
      ...
      await voiceAssistant.donateIntent('donate-intent', { task: 'Complete report' });

      expect(mockModule.donateIntent).toHaveBeenCalledWith(
        'donate-intent',
    ...
    it('should throw when donating non-registered intent', async () => {
      await expect(voiceAssistant.donateIntent('unknown', {}))
        .rejects.toThrow('Intent unknown not registered');
    });
  });
  ...
  describe('Permissions', () => {
    ...
    it('should request microphone permission', async () => {
      mockModule.requestMicrophonePermission.mockResolvedValue(PermissionStatus.GRANTED);
    ...
    it('should request speech recognition permission', async () => {
      mockModule.requestSpeechRecognitionPermission.mockResolvedValue(PermissionStatus.GRANTED);
  ...
  describe('Event Handling', () => {
    ...
    it('should add and trigger event listeners', async () => {
      ...
      const receivedHandler = mockModule.addListener.mock.calls
        .find(call => call[0] === 'onIntentReceived')?.[1];
    ...
    it('should remove event listeners', () => {
      const unsubscribe = voiceAssistant.addEventListener('onIntentCompleted', eventHandler);

      unsubscribe();

      const completedHandler = mockModule.addListener.mock.calls
        .find(call => call[0] === 'onIntentCompleted')?.[1];
    ...
    it('should handle multiple listeners for same event', async () => {
      ...
      const failedHandler = mockModule.addListener.mock.calls
        .find(call => call[0] === 'onIntentFailed')?.[1];
