...
describe("Voice Commands Integration", () => {
  ...
  describe("Search Voice Commands", () => {
    it("should implement complete search flow with voice commands", async () => {
      ...
      const searchIntent = VoiceIntentBuilder.create<{
        query: string;
    ...
    it("should handle disambiguation when parameters are missing", async () => {
      ...
      const missingParamResult = await voiceAssistant.executeIntent(
        "smart-search",
        {}
      );
      ...
      const completeResult = await voiceAssistant.executeIntent(
        "smart-search",
        {
          query: "test query",
        }
  ...
  describe("Media Control Voice Commands", () => {
    it("should implement media playback controls", async () => {
      ...
      const createMediaIntent = (action: "play" | "pause" | "skip") => {
        return VoiceIntentBuilder.create<{ trackName?: string }>()
          .withId(`media-${action}`)
  ...
  describe("Todo List Voice Commands", () => {
    it("should handle todo list operations", async () => {
      ...
      const todoIntent = VoiceIntentBuilder.create<{
  ...
  describe("Exercise Tracking Voice Commands", () => {
    it("should handle workout intents with health integration", async () => {
      ...
      const exerciseIntent = VoiceIntentBuilder.create<{
      ...
      const startResult = await voiceAssistant.executeIntent(
        "fitness-tracker",
        {
          action: "start",
  ...
  describe("Error Handling and Recovery", () => {
    it("should handle and recover from service failures", async () => {
      ...
      const resilientIntent = VoiceIntentBuilder.create<{ retry?: boolean }>()
  ...
  describe("Permission Flow", () => {
    it("should handle complete permission request flow", async () => {
      ...
      mockModule.requestSpeechRecognitionPermission.mockResolvedValueOnce(
      ...
      const speechStatus =
        await voiceAssistant.requestSpeechRecognitionPermission();
      expect(speechStatus).toBe(PermissionStatus.GRANTED);

      const voiceIntent = VoiceIntentBuilder.create()
